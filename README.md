...
- **Keep the item count low.** Every bar item is a window macOS rebuilds when displays sleep/wake, so a heavy bar goes unresponsive for up to a minute after unlocking. Measured on this setup (3 displays): 148 items → 30–95s stall; 55 items → none. A 3-item probe bar with identical geometry never stalled, which is how the item count was identified as the axis. Workspace items are therefore created **on demand** (a workspace gets a pill/bracket/padding only while it has windows or is focused) instead of creating all 33 and hiding them.
